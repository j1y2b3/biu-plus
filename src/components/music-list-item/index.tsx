...
import { RiPlayFill, RiScissorsLine } from "@remixicon/react";
...
import SongTrimModal, { type SongTrimModalTarget } from "../song-trim-modal";
...
const MusicListItem = ({
  ...
  // 单曲裁剪：有 bvid 或 sid 的歌曲支持设置跳过开头/结尾
  const canTrim = Boolean(bvid) || sid !== undefined;
  ...
  const handleMenuAction = (key: string) => {
    if (key === "trim") {
      setTrimTarget({ type, bvid, sid, title: String(title) });
      return;
    }
    onMenuAction?.(key);
  };
        ...
        >
          <div className={clx("grid w-full items-center gap-4", gridCols)}>
            ...
            {isCompact ? (
              ...
              <div className="flex min-w-0 items-center overflow-hidden">
                ...
                <div className="ml-2 flex min-w-0 flex-col items-start justify-center space-y-1">
                  <span className={clx("w-full truncate text-left text-base", { "text-primary": isPlay })}>
                    {title}
                  </span>
                  {Boolean(upName) && (
                    <span
            ...
            <div className="flex h-full items-center justify-end">
              {Boolean(allMenus.length) && <OperationMenu items={allMenus} onAction={handleMenuAction} />}
            </div>
          </div>
        </Button>
      </ContextMenu>
